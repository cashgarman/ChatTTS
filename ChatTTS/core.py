...
class Chat:
	...
	def infer(
		...
		print(f"Detect language and init normalizer time: {time.time() - timer}")
		
		timer = time.time()
		for i, t in enumerate(text):
			invalid_characters = count_invalid_characters(t)
			if len(invalid_characters):
				self.logger.log(logging.WARNING, f'Invalid characters found! : {invalid_characters}, full text: {text}, portion: {t}')
				text[i] = apply_character_map(t)
		print(f"Count invalid characters and apply character map time: {time.time() - timer}")
		...
		if not skip_refine_text:
			...
			if refine_text_only:
				return text
			print(f"Refine text time: {time.time() - timer}")
		
		timer = time.time()
		text = [params_infer_code.get('prompt', '') + i for i in text]
		params_infer_code.pop('prompt', '')
		result = infer_code(self.pretrain_models, text, **params_infer_code, return_hidden=use_decoder, max_new_token=1024)
		print(f"Infer code time: {time.time() - timer}")
		...
		print(f"use_decoder time: {time.time() - timer}")
		
		timer = time.time()
		wav = [self.pretrain_models['vocos'].decode(i).cpu().numpy() for i in mel_spec]
		print(f"Decode time: {time.time() - timer}")
	...
	def sample_random_speaker(self, ):
		...
		print(f"Sample random speaker time: {time.time() - timer}")
